...
public partial class admin_ViewJobs : System.Web.UI.Page
{
    ...
    protected void del_news_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            com.Parameters.AddWithValue("@jid", lb.CommandArgument);
            com.ExecuteNonQuery();
            con.Close();
            Page.ClientScript.RegisterStartupScript(GetType(), "", "<script>alert('Jobs Deleted '); location.replace('ViewJobs.aspx')</script>");
